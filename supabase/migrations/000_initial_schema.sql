...
CREATE POLICY "사용자는 자신의 방문 요청 읽기" ON visit_requests
  FOR SELECT USING (auth.uid() = requester_id OR has_role(auth.uid(), 'admin'));
...
CREATE POLICY "방문자 정보 읽기" ON visitor_info
  FOR SELECT USING (
    EXISTS (
      ...
      AND (visit_requests.requester_id = auth.uid() OR has_role(auth.uid(), 'admin'))
    )
  );
